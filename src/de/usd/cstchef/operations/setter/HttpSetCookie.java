	...
	protected byte[] perform(byte[] input) throws Exception {
		...
		try {
			...
			if( (offset > 0) && isRequest ) {
				...
				int bodyOffset = resp.getBodyOffset() - 2;
				byte[] value = new byte[cookieName.length + newValue.length + cookieHeaderLength + 4];
				...
				System.arraycopy(";\r\n".getBytes(), 0, value, cookieHeaderLength + cookieName.length + 1 + newValue.length, 3);
